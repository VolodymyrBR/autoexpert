(() => {
    ...
    (() => {
        ...
        window.onload = function() {
            ...
            const catalogPrices = document.querySelectorAll(".info__price");
            let minPrice = 1 / 0;
            let maxPrice = -1 / 0;
            catalogPrices.forEach((price => {
                const priceValue = parseFloat(price.textContent.replace(/\s+/g, ""));
            ...
            if (null !== priceSlider) {
                ...
                priceSlider.noUiSlider.on("update", (function(values, handle) {
                    ...
                    if (0 === handle) priceStart.value = Math.round(value);
                    if (1 === handle) priceEnd.value = Math.round(value);
                }));
        ...
        function initSliders() {
            ...
            if (document.querySelector(".main-card__image")) {
                let thumbsSwiper = new core(".image-preview__slider", {
                    ...
                    navigation: {
                        prevEl: ".image-preview-prev",
                        nextEl: ".image-preview-next"
                    },
                    on: {}
                });
                new core(".image-product__slider", {
                    ...
                    navigation: {
                        prevEl: ".image-product-prev",
                        nextEl: ".image-product-next"
                    },
                    on: {}
                });
            }
...
***************************************************************************** */
        ...
        document.addEventListener("click", (event => {
            const button = event.target.closest("[data-cart-in]");
            if (button) {
                const infoBlock = button.closest(".product-card__info, .info-card__action");
                if (infoBlock) {
                    const isInCart = infoBlock.classList.contains("_cart-in");
                    if (isInCart) {
                        infoBlock.classList.remove("_cart-in");
                        cartCount--;
